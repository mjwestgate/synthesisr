...
bib <-
  read_refs(c(tmp1, tmp2, tmp3), return_df = FALSE, verbose = TRUE)
df <-
  read_refs(c(tmp1, tmp2, tmp3), return_df = TRUE, verbose = TRUE)


expect(
  format_citation(bib)[1] == "Haddaway, Neal R. et al. (2019) Eviatlas: a Tool for Visualising Evidence Synthesis Databases. Environmental Evidence.",
  "Output citation does not match expected format for data.frame"
)

expect(
  format_citation(df)[1] == "Haddaway, Neal R. et al. (2019) Eviatlas: a Tool for Visualising Evidence Synthesis Databases. Environmental Evidence.",
  "Output citation does not match expected format for data.frame"
)

expect(
  all(format_citation(df) == format_citation(bib)),
  "citations for objects of class data.frame and bibliography not being formatted the same"
)

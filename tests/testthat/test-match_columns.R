...
expect(all(colnames(match_columns(my_df))==c("database", "author", "year", "title")),
       "Column names not being matched in proper order to code lookup")


my_df2 <- data.frame(PY=2019,
...
expect(all(colnames(match_columns(my_df2))==c("database", "author", "year", "title", "doi", "random_noise")),
